...
-- newtype Grammar = Grammar [RuleDecl] deriving (Eq, Show)
-- empty :: Grammar
-- empty = Grammar []
...
-- newtype RuleDef = RuleDef Alt deriving (Eq, Show)

-- newtype RuleRef = RuleRef BS.ByteString deriving Show
...
--     ]
--     where decl r d = RuleDecl (ref r) (RuleDef d)
...
-- ruleToParseable :: Grammar -> RuleRef -> ABNFParseable
-- ruleToParseable grammar r =
--     case getMergedDecl grammar r of
--         Nothing -> error $ "non-existing rule " ++ show r
...
-- elemParser def =
--     case def of
--         RuleRefE r -> ruleParser (toRef r)
--         GroupE g -> groupParser g
--         OptE o -> optParser o
--         TermE term -> termParser term
--         _ -> Parseable.Unit "fail" (fail "fail")
